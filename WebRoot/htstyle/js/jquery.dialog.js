(function ($) {
    ...
    $.uedWidget("ued.Dialog", {
        ...
        _createDialogDom: function ($el, options, _define) {
            var $farther = (_define.$farther = $el.parent()),
                buttons = options.buttons;
            var $dialog = (_define.$dialog = $(
                '<div class="ui-dialog-panel"><div class="ui-dialog"></div><div class="ui-dialog-leftYY"></div><div class="ui-dialog-rightYY"></div><div class="ui-dialog-bottomLeft"></div><div class="ui-dialog-bottomRight"></div><div class="ui-dialog-bottomCenter"></div></div>'
            ));
            //创建弹窗头部区域
            ...
            if (buttons) {
                ...
                for (var i = 0, len = buttons.length; i < len; i++) {
                    var button = buttons[i],
                        length = this._returnLetterLength(button.label);
        ...
        _dialogDrag: function (self, _define) {
            var $dragMask = _define.$dragMask,
                $contentMask = _define.$contentMask;
            var $dialogHd = _define.$dialogHd,
                $dialog = _define.$dialog,
                winWidth,
                winHeight;
            var left,
                top,
                dialogWidth = _define.width,
                dialogHeight = _define.height;
            var tempX, tempY;
        ...
        setTitle: function (_title) {
            var _define = this._define,
                $dialogHd = _define.$dialogHd;
            if (_title) {
                $dialogHd
                    .find("h6")
                    .empty()
                    .append('<span class="dialog-hd-lc"></span>' + _title + '<span class="dialog-hd-rc"></span>');
            }
        },
        ...
        setWidth: function (_width) {
            var _define = this._define,
                $dialogContent = _define.$dialogContent;
        ...
        setMessage: function (_message) {
            if (!_message) return;
            var _define = this._define,
                $dialogMessage = _define.$dialogMessage;
        ...
        _returnLetterLength: function (_str) {
            return _str.replace(/[a-z0-9]{2}/gi, "a").length;
        },
        remove: function () {
