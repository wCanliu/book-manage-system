    ...
    UM.registerWidget(widgetName, {
        ...
        initRequestApi: function () {
            ...
            if (window.BMap && window.BMap.Map) {
                ...
                $ifr.write(
                    this.root()
                        .find(".edui-tpl-container")
                        .html()
                        .replace(/scr_ipt/g, "script")
                        .replace("<<id>>", "'" + this.editor.id + "'")
                );
            }
        },
        ...
        initBaiduMap: function () {
            ...
            if (img.length && /api[.]map[.]baidu[.]com/gi.test(img.attr("src"))) {
                ...
                exec: function (editor) {
                    ...
                    if (widget.root().find(".edui-map-dynamic")[0].checked) {
                        var URL = editor.getOpt("UMEDITOR_HOME_URL"),
                            url = [
                                URL + (/\/$/.test(URL) ? "" : "/") + "dialogs/map/map.html" + "#center=" + center.lng + "," + center.lat,
                                "&zoom=" + zoom,
                                "&width=" + size.width,
                                "&height=" + size.height,
                        ...
                        editor.execCommand(
                            "inserthtml",
                            '<iframe class="ueditor_baidumap" src="' + url + '" frameborder="0" width="' + (size.width + 4) + '" height="' + (size.height + 4) + '"></iframe>'
                        );
                    } else {
                        url =
                        ...
                        editor.execCommand(
                            "inserthtml",
                            '<img width="' + size.width + '"height="' + size.height + '" src="' + url + '"' + (widget.imgcss ? ' style="' + widget.imgcss + '"' : "") + "/>",
                            true
                        );
                    }
