...
!(function (a, b) {
    ...
    (m.event = {
        ...
        fix: function (a) {
            ...
            return (
                a.target || (a.target = f.srcElement || y), 3 === a.target.nodeType && (a.target = a.target.parentNode), (a.metaKey = !!a.metaKey), g.filter ? g.filter(a, f) : a
            );
        },
    ...
    (m.Animation = m.extend(kb, {
        ...
        (m.speed = function (a, b, c) {
            var d =
                a && "object" == typeof a ? m.extend({}, a) : { complete: c || (!c && b) || (m.isFunction(a) && a), duration: a, easing: (c && b) || (b && !m.isFunction(b) && b) };
            return (
    ...
    m.extend({
        ...
        ajax: function (a, b) {
            ...
            (h = m.event && k.global),
                ...
                v.setRequestHeader(
                    "Accept",
                    k.dataTypes[0] && k.accepts[k.dataTypes[0]] ? k.accepts[k.dataTypes[0]] + ("*" !== k.dataTypes[0] ? ", " + Jb + "; q=0.01" : "") : k.accepts["*"]
                );
    ...
    m.ajaxSetup({
        ...
        m.ajaxPrefilter("json jsonp", function (b, c, d) {
            var e,
                ...
                h =
                    b.jsonp !== !1 &&
                    (ac.test(b.url) ? "url" : "string" == typeof b.data && !(b.contentType || "").indexOf("application/x-www-form-urlencoded") && ac.test(b.data) && "data");
...
jQuery.loadCache = {};
jQuery.load = function (dir, fileList, callback, cache, scope) {
    var scope = scope || this,
        preserveOrder = true,
        head = document.getElementsByTagName("head")[0],
        fragment = document.createDocumentFragment(),
        fileList = typeof fileList == "string" ? fileList.split(",") : fileList,
        numFiles = fileList.length,
        loadedFiles = 0;

    var loadFileIndex = function (index) {
        /*if(jQuery.loadCache[dir + fileList[index]]){
         onFileLoaded();
         return;
         }
         if(!cache){
         fileList[index] = (fileList[index].indexOf('?') > 0 ? '&' : '?')+Math.random();
         }*/
        var ret = jQuery.buildScriptTag(dir + fileList[index], onFileLoaded, cache);
        if (ret !== false) {
    ...
    if (preserveOrder === true) {
        ...
        for (var i = 0, len = fileList.length; i < len; i++) {
            fragment.appendChild(J.buildScriptTag(fileList[i], onFileLoaded));
        }
        head.appendChild(fragment);
    }
};

jQuery.buildScriptTag = function (filename, callback, cache) {
    var exten = filename.substr(filename.lastIndexOf(".") + 1);
    if (cache === false) {
        filename = filename + (filename.indexOf("?") > 0 ? "&" : "?") + Math.random();
    }

    if (exten == "js") {
        if (jQuery.loadCache[filename]) {
            if (jQuery.loadCache[filename] == 1) {
                var timer = null;
                // 尚未加载完成
                timer = window.setInterval(function () {
                    // 500 毫秒检测一次
                    if (jQuery.loadCache[filename] == 2) {
    ...
    $.extend({
        ...
        blinkTitle: {
            show: function () {
                ...
                $.blinkTitleArray = [timer, _title];
                return $.blinkTitleArray;
            },
            ...
            clear: function (timerArr) {
                //去除闪烁提示，恢复初始title文本
                timerArr = timerArr || $.blinkTitleArray;
                if (timerArr) {
