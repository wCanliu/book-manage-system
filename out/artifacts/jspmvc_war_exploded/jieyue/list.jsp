        ...
        <div class="panel-body">
            <div class="form-search pa10 bg-warning">
                <form class="form-inline" action="?" size="small" id="formSearch">
                    ...
                    分类：
                    <select class="form-control class_fenlei9" data-value="${param.fenlei}" id="fenlei" name="fenlei">
                    ...
                    <tbody>
                        <c:set var="i" value="0" /><c:forEach items="${lists}" var="map"
                            ...
                            <tr id="${map.id}" pid="">
                                ...
                                <td>${map.mingcheng}</td>
                                <td><ssm:sql var="mapfenlei5" type="find">SELECT fenleiming,id FROM fenlei where id='${map.fenlei}'</ssm:sql>${mapfenlei5.fenleiming}</td>
                                <td>${map.zuozhe}</td>
                                <td>${map.yonghu}</td>
                                <td>${map.kaishi}</td>
                                <td>${map.jieshu}</td>
                                ...
                                <td align="center">
                                    <a class="btn btn-success btn-xs" href="jieyue.do?ac=updt&id=${map.id}" title="编辑"> 编辑 </a>

                                    <a class="btn btn-danger btn-xs" href="jieyue.do?ac=delete&id=${map.id}" onclick="return confirm('确定删除？此操作不可恢复')" title="删除">
                                        删除
                                    </a>
